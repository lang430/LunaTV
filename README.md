...
> 关键前置：Cloudflare 构建需要本地安装适配层（`open-next` / `@cloudflare/next-on-pages`）。安装会更新 `pnpm-lock.yaml`，该变更一并提交后，render 的 `pnpm install --frozen-lockfile` 同样包含这些 devDependency，不影响现有 Docker / render 部署。
