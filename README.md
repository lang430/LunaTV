...
MoonTV 支持部署到 Cloudflare，提供两种方案。**在 Cloudflare 上无法使用 TCP Redis / Kvrocks**，必须使用 **Upstash（HTTP Redis）** 作为存储后端（本项目已内置支持）。

#### 方案 A：Cloudflare Workers（推荐，使用 OpenNext）

```bash
# 1. 安装适配层（仅首次，或用脚本内的 pnpm dlx 自动拉取）
pnpm add -D @opennextjs/cloudflare wrangler

# 2. 构建（CF_BUILD=1 会让 next.config 关闭 standalone、保留原生 crypto）
pnpm cf:workers:build
# 等价于：CF_BUILD=1 pnpm dlx @opennextjs/cloudflare build

# 3. 注入 Upstash 凭据（敏感信息用 Secret，不要写进 wrangler.jsonc）
pnpm dlx wrangler secret put UPSTASH_URL
pnpm dlx wrangler secret put UPSTASH_TOKEN

# 4. 部署
pnpm cf:workers:deploy
# 等价于：pnpm dlx wrangler deploy
```
...
# 2. 构建
pnpm cf:pages:build
# 等价于：CF_BUILD=1 pnpm dlx @cloudflare/next-on-pages

# 3. 注入 Upstash 凭据
pnpm dlx wrangler pages secret put UPSTASH_URL
pnpm dlx wrangler pages secret put UPSTASH_TOKEN

# 4. 部署
pnpm cf:pages:deploy
# 等价于：pnpm dlx wrangler pages deploy .vercel/output
```
...
| 变量 | 说明 | 示例 |
| --- | --- | --- |
| `NEXT_PUBLIC_STORAGE_TYPE` | 固定为 `upstash` | `upstash` |
| `UPSTASH_URL` | Upstash Redis REST 地址（Secret） | `https://xxx.upstash.io` |
| `UPSTASH_TOKEN` | Upstash Redis Token（Secret） | `AXxx...` |

> 其余 MoonTV 运行时变量（如站点设置相关）也通过 `wrangler secret put` 注入。

#### 兼容性与注意事项

- **运行时**：所有 API route 声明 `runtime = 'nodejs'`，Cloudflare 通过 `compatibility_flags = ["nodejs_compat"]` 兼容；`next.config.js` 中 `output: 'standalone'` 与 `crypto: false` 仅在非 Cloudflare 构建（`CF_BUILD` 未设置）时生效，不影响现有 Docker / Zeabur 部署。
- **存储**：务必使用 Upstash；TCP Redis / Kvrocks 在 Cloudflare 上不可用。
- **PWA**：`next-pwa` 生成的 `public/sw.js` 会作为静态资源随构建发布，通常无需额外处理。
- **实时搜索（WebSocket）**：`/api/search/ws` 依赖 WebSocket 升级，在 Cloudflare 上可能受平台限制，建议在 Cloudflare 部署场景下评估或改用普通搜索接口。
- **兼容性日期**：`compatibility_date` 设为 `2025-04-01`（可按需上调），确保 `nodejs_compat` 行为稳定。
